...
WavFileHeader default_wav_header() {
    return (WavFileHeader){ .Header = default_header(), .Format = default_format() };
}

WavFileHeader read_wav_file(FILE* fp) {
    return (WavFileHeader){ .Header = read_header_file(fp), .Format = read_format_file(fp) };
}
...
WavFileHeader read_wav_arr(uint32_t* buf, size_t len) {
    ...
    return (WavFileHeader){
        ...
        .Format = read_format_arr(buf + sizeof(Header), len - sizeof(Header))
    };
}
...
int verify_format(FILE* fd, WavFileHeader header) {
  if (strncmp((char*)header.Header.Id.b, "RIFF", (int)sizeof(header.Header.Id.w)) != 0) {
    fprintf(fd, "File must be RIFF format, may have been error reading stream [Header.Id=%.*s]", (int)sizeof(header.Header.Data.w), header.Header.Id.b);
    return EXIT_FAILURE;
  }

  if (strncmp((char*)header.Header.Data.b, "WAVE", (int)sizeof(header.Header.Data.w)) != 0) {
    fprintf(fd, "Error reading stream, expected header data to contain 'WAVE' [Header.Data=%.*s]", (int)sizeof(header.Header.Data.w), header.Header.Data.b);
    return EXIT_FAILURE;
  }
